-- ============================================================
-- Autopilot: tabelas para automação completa de conteúdo
-- ============================================================

-- 1. Configurações do autopilot (uma por marca por usuário)
...
-- 2. Calendários (ciclos de conteúdo gerado)
...
create policy "Users manage own autopilot calendars"
  on public.autopilot_calendars for all
  using (auth.uid() = user_id);

-- 3. Posts individuais do autopilot
...
create policy "Users manage own autopilot posts"
  on public.autopilot_posts for all
  using (auth.uid() = user_id);

-- 4. Coluna Firecrawl API key em user_configs
